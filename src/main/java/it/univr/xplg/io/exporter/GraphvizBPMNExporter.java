 ...
 * @see plg.io.exporter.GraphvizBPMNExporter
 */
@Exporter(
...
public class GraphvizBPMNExporter extends FileExporter
{
     ...
     */
    private void preprocessString(ProcessWithImpacts model) {
        // start events
        String buffer = "";
        ...
        for(Task e : model.getTasks())
        {
            ...
            if (e instanceof TaskWithImpacts)
            {
                ...
                if ((impatti!=null)&&(impatti.size()>0))
                {
                    ...
                    impTag.replace(impTag.length()-1,impTag.length(),")");
                    impTag.append("\" ");
                    impactRepr = impTag.toString();
                }
            }
            buffer += "\tc_" + e.getId() + " [label=\"" + e.getName() + "\" "+impactRepr+"];\n";
        }
        ...
        for(DataObject o : model.getDataObjects()) {
            if (o.getObjectOwner() != null) {
                if (o.getDirectionOwner() == IDataObjectOwner.DATA_OBJECT_DIRECTION.REQUIRED) {
